...
// Строит строку из цветных блоков,
// При наведении показывает время и задержку
...
// Создаёт новый график на canvas с заданным id
...
function updateChart(chart, history) {
  ...
  chart.update('none'); // без анимаиции
}


// Генерирует HTML карточки сайта
...
// Запрашивает список сайтов и их истории с бэка.
// При первом вызове создаёт карточки,
// Дальше обновляет данные в существующих карточках
...
// Отрисовывает карточки пользовательских сайтов
...
// Делает разовую проверку пользовательского сайта через /api/uptime/check
// и добавляет результат в историю
